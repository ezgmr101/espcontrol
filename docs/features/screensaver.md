...
You can also enable **Always show cover art** and enter a media player entity, such as `media_player.living_room`. When that media player is in the `playing` state, the timer keeps waiting instead of putting the screen to sleep.
